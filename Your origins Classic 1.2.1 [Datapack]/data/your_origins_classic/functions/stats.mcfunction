...
scoreboard players set @a[scores={stats.vegan=4..}] stats.vegan 3
...
effect give @a[advancements={your_origins_classic:not_vegan_eat_vegetarian_food=true}] poison 10 1 false
advancement revoke @a only your_origins_classic:not_vegan_eat_vegetarian_food
...
execute as @a[scores={stats.afraidWater=1},predicate=jabka_core:in_water] at @s run damage @s 4 minecraft:drown
...
### unDead
scoreboard players set @a[scores={stats.unDead=..0}] stats.unDead 0
scoreboard players set @a[scores={stats.unDead=2..}] stats.unDead 1

#if stats.unDead = 0, we do nothing ;)
execute as @a[scores={stats.unDead=1}] at @s[predicate=jabka_core:day,predicate=!jabka_core:raining,predicate=!jabka_core:in_water,nbt={Dimension:"minecraft:overworld"}] unless block ~ ~ ~ water_cauldron[level=3] positioned over motion_blocking if entity @s[dy=999] run damage @s 4 on_fire