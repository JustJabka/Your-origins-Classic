...
execute as @a[scores={stats.unDead=1}] at @s[predicate=jabka_core:day,predicate=!jabka_core:raining,predicate=!jabka_core:not_raining_biome,predicate=!jabka_core:in_water,nbt={Dimension:"minecraft:overworld"}] positioned over motion_blocking if entity @s[dy=999] run damage @s 4 on_fire